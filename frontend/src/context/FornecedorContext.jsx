...
export const FornecedorProvider = ({ children }) => {
  ...
  const getFornecedor = async (id_fornecedor) => {
    try {
      ...
      console.error(
        `Erro ao carregar fornecedor com ID ${id_fornecedor}:`,
        error
      );
      throw error;
    }
  ...
  const addFornecedor = async (newFornecedor) => {
    try {
      ...
      return {
        success: false,
        message:
          "Erro ao adicionar o fornecedor. Verifique os dados e tente novamente.",
      };
    }
  };
    ...
    try {
      ...
      setFornecedores(
        fornecedores.map((fornecedor) =>
          fornecedor.id_fornecedor_fornecedor === id_fornecedor
            ? updated
            : fornecedor
        )
      );
  ...
  const removeFornecedor = async (id_fornecedor) => {
    try {
      ...
      setFornecedores(
        fornecedores.filter(
          (fornecedor) => fornecedor.id_fornecedor_fornecedor !== id_fornecedor
        )
      );
    } catch (error) {
