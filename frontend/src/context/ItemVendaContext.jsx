...
import { createItemVenda, getAllItemVenda, getItemVendaById } from '../services/itemVendaService';
...
export const ItemVendaProvider = ({ children }) => {
  ...
  const getItemVenda = async (id_item_venda) => {
    try {
      ...
      if (!itemvenda) throw new Error(`Venda com ID ${id_item_venda} não encontrada.`);
      return itemvenda;
    } catch (error) {
  ...
  const addItemVenda = async (itemvenda) => {
    try {
      ...
      return { success: false, message: 'Erro ao atualizar o item venda. Verifique os dados e tente novamente.' };
    }
  };

  return (
    <ItemVendaContext.Provider value={{ itemVenda, loading, addItemVenda, getItemVenda }}>
      {children}
