...
const AddItemVenda = () => {
  ...
  const [itemvendaData, setItemVendaData] = useState({
    id_venda: "",
    id_produto: "",
    quantidade: "",
    preco_unitario: "",
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!itemvendaData.id_venda)
      newErrors.id_venda = "Id de Venda é obrigatório";
    if (!itemvendaData.id_produto)
      newErrors.id_produto = "Id do Produto é obrigatório";
    if (!itemvendaData.quantidade || itemvendaData.quantidade <= 0)
      newErrors.quantidade = "Quantidade inválido";
    if (!itemvendaData.preco_unitario || itemvendaData.preco_unitario <= 0)
      newErrors.preco_unitario = "Preço Unitário inválido";
    if (!itemvendaData.subtotal || itemvendaData.subtotal <= 0)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors((prevErrors) => ({
        ...prevErrors,
        id_venda: "ID de Venda inválido ou não encontrado.",
      }));
    ...
    try {
      ...
      setErrors((prevErrors) => ({
        ...prevErrors,
        id_produto: "ID do Produto inválido ou não encontrado.",
      }));
      validationErrorExists = true;
    }
    ...
    try {
      ...
      setErrors({
        form:
          error.message || "Erro ao adicionar o Item Venda. Tente novamente.",
      });
    } finally {
      setLoading(false);
    }
              ...
              <Card className="item-venda-card">
                ...
                <Card.Body className="item-venda-card-body">
                  ...
                  {success && (
                    <Alert
                      variant="success"
                      className="item-venda-alert-success"
                    >
                      {success}
                    </Alert>
                  )}
                  <Form onSubmit={handleSubmit}>
                    <Row>
                      <Col md={6}>
                        <Form.Group
                          className="item-venda-form-group"
                          controlId="id_venda"
                        >
                          <Form.Label className="item-venda-form-label">
                            ID da Venda
                          </Form.Label>
                          <Form.Control
                    ...
                    <div className="button-container">
                      <Button
                        className="item-venda-button-container"
                        variant="primary"
                        type="submit"
                        disabled={loading}
                      >
