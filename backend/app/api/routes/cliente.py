...
from app.services.services_cliente import (
    get_all_clientes,
    get_cliente_by_id,
    create_cliente,
    update_cliente,
...
def view_cliente_route(
    ...
    try:
        logger.info(
            f"Detalhes de Cliente recuperado com sucesso pelo usuário: {current_user.username}"
        )
...
def add_cliente_route(
    ...
    try:
        ...
        logger.error(
            f"Erro HTTP ao criar o cliente {cliente.nome_cliente}: {he.detail}"
        )
        raise he
    except Exception as e:
...
def update_cliente_route(
    ...
    try:
        logger.info(
            f"Cliente atualizado com sucesso pelo usuário: {current_user.username}"
        )
...
def delete_cliente_route(
    ...
    try:
        logger.info(
            f"Cliente deletado com sucesso pelo usuário: {current_user.username}"
        )
