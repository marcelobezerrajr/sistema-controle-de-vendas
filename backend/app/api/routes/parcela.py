...
def view_item_venda_route(id_parcela: int, db: Session = Depends(get_db), current_user: User = Depends(get_read_user_admin)):
    try:
        logger.info(f"Parcela listada com sucesso pelo usuário: {current_user.username}")
