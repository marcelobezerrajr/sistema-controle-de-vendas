...
from app.services.services_venda import (
    get_all_vendas,
    get_venda_by_id,
    create_venda,
    update_venda,
...
from app.schemas.schemas_venda import Venda, VendaCreate, VendaUpdate
...
def list_vendas_route(
    db: Session = Depends(get_db), current_user: User = Depends(get_read_user_admin)
):
    try:
        logger.info(
            f"Vendas listadas com sucesso pelo usuário: {current_user.username}"
        )
...
def update_venda_route(
    ...
    try:
        logger.info(
            f"Venda atualizada com sucesso pelo usuário: {current_user.username}"
        )
